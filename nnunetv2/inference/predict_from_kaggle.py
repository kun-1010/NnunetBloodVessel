...
# 设置环境变量
os.environ["nnUNet_preprocessed"] = "/home/user/Kaggle/Data/BloodVessel/nnUNet_preprocessed"
os.environ["nnUNet_results"] = "/home/user/Kaggle/Data/BloodVessel/nnUNet_results"
os.environ["nnUNet_raw"] = "/home/user/Kaggle/Data/BloodVessel/nnUNet_raw"
...
class nnUNetPredictor(object):
    def __init__(self,
        ...
        # 内部变量
        self.plans_manager = None  # 计划管理器
        self.configuration_manager = None  # 配置管理器
        self.list_of_parameters = None  # 参数列表
        self.network = None  # 网络
        self.dataset_json = None  # 数据集 JSON 文件
        self.trainer_name = None  # 训练器名称
        self.allowed_mirroring_axes = None  # 允许镜像的轴
        self.label_manager = None  # 标签管理器

        # 预测设置
        self.tile_step_size = tile_step_size  # 滑动窗口预测步长
        self.use_gaussian = use_gaussian  # 是否使用高斯滤波进行后处理
        self.use_mirroring = use_mirroring  # 是否在预测时使用镜像数据增强
        if device.type == 'cuda':
            # 修复避免 DDP 推理问题
            # pass
            pass
        if device.type != 'cuda':
            print(f'perform_everything_on_gpu=True 仅在 cuda 设备上支持！将其设置为 False')
            perform_everything_on_gpu = False
        self.device = device  # 设备
        self.perform_everything_on_gpu = perform_everything_on_gpu  # 是否在所有操作上都使用 GPU
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        """
            功能：从训练模型文件夹初始化预测器。
            输入：
            model_training_output_dir：训练模型文件夹路径。
            use_folds：要用于预测的折叠列表。默认为 None，将自动检测可用折叠。
            checkpoint_name：要加载的检查点文件名。默认为 'checkpoint_final.pth'。
        """
    ...
    def manual_initialization(self, network: nn.Module, plans_manager: PlansManager,
        ...
        """
            ...
            parameters：参数列表。
            dataset_json：数据集JSON文件。
            trainer_name：训练器名称。
            inference_allowed_mirroring_axes：允许镜像的轴。
    ...
    def predict_from_files(self,
                            ...
                            part_id: int = 0):
        """
            这是 nnU-Net 用于进行预测的默认函数。它最适合于批量预测（一次预测许多图像）
            list_of_lists_or_source_folder：指定输入数据的路径或列表。
            output_folder_or_list_of_truncated_output_files：指定输出文件的路径或列表。
            save_probabilities：是否保存预测的概率图。
            overwrite：是否覆盖已存在的输出文件。
            num_processes_preprocessing：用于预处理数据的进程数。
            num_processes_segmentation_export：用于将预测结果转换为分割结果并保存的进程数。
            folder_with_segs_from_prev_stage：指定前一阶段的分割结果文件夹路径，仅在使用级联网络时需要。
            num_parts：将预测任务分成多少个部分，用于并行处理大数据集。
            part_id：指定当前处理的是哪个部分，用于并行处理时区分任务。
        """
        if isinstance(output_folder_or_list_of_truncated_output_files, str):
        ...
        ########################
        # 存储输入参数，以便清楚地了解用于生成预测的内容
        if output_folder is not None:
            my_init_kwargs = {}
            for k in inspect.signature(self.predict_from_files).parameters.keys():
                my_init_kwargs[k] = locals()[k]
            my_init_kwargs = deepcopy(
                my_init_kwargs)  # 不要无意中改变任何内容。将此视为递归修复 JSON 导出的方法
            recursive_fix_for_json_export(my_init_kwargs)
            maybe_mkdir_p(output_folder)
            save_json(my_init_kwargs, join(output_folder, 'predict_from_raw_data_args.json'))

            # 如果想要对预测进行处理，例如应用后处理，则需要这两个
        ...
        if self.configuration_manager.previous_stage_name is not None:
            assert folder_with_segs_from_prev_stage is not None, \
                f'请求的配置是级联网络。它需要前一阶段 ({self.configuration_manager.previous_stage_name}) ' \
                f'的分割结果作为输入。请通过 folder_with_segs_from_prev_stage 提供它们所在的文件夹'
...
#         return self.predict_from_data_iterator(data_iterator, save_probabilities, num_processes_segmentation_export)
    ...
    def predict_from_data_iterator(self,
                                      ...
                                      num_processes_segmentation_export: int = default_num_processes):
        """
        每个数据迭代器返回的元素必须是一个带有 `data`、`ofile` 和 `data_properties` 键的字典！
        如果 `ofile` 为 `None`，则会返回结果而不是写入文件
        """
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            worker_list = [i for i in export_pool._pool]
            r = []
            names = []
            for i,preprocessed in enumerate(data_iterator):
                data = preprocessed['data']
                names.append(preprocessed['name'])
                ...
                if ofile is not None:
                    print(f'正在预测 {os.path.basename(ofile)}:')
                else:
#                     print(f'\n正在预测形状为 {data.shape} 的图像:{os.path.basename(ofile)}')
                    print(f'正在预测形状为 {data.shape} 的图像:')

                print(f'perform_everything_on_gpu: {self.perform_everything_on_gpu}')

                properties = preprocessed['data_properties']

                # 防止 GPU 预测速度过快，导致磁盘被 npy 文件淹没
...
#                     name.append(os.path.basename(ofile))

                if ofile is not None:
                    print(f'完成 {os.path.basename(ofile)}')
                else:
                    print(f'已完成形状为 {data.shape} 的图像:\n')

            ret = [i.get()[0] for i in r]
            
        if len(names) != len(ret):
            raise ValueError("两个列表的长度不相等。")

        dict_mask = {}
        for i in range(len(names)):
            dict_mask[names[i]] = ret[i]
...
if __name__ == '__main__':
    ...
    if nnUNet_results is None:
        raise ValueError("变量 nnUNet_results 为空，请检查赋值情况.")
    else:
    ...
    print(os.path.join(nnUNet_raw, 'Dataset001_BloodVessel_2D/imagesTs'))
    print(f"Predictor 初始化路径: {os.path.join(nnUNet_results, 'Dataset001_BloodVessel_2D/nnUNetTrainer__nnUNetPlans__2d')}")

    predictor.initialize_from_trained_model_folder(
        os.path.join(nnUNet_results, 'Dataset001_BloodVessel_2D/nnUNetTrainer__nnUNetPlans__2d'),
        use_folds=(0,),
        checkpoint_name='checkpoint_best.pth',
    )

    ret, r, dict_mask = predictor.predict_from_files(
        # os.path.join(nnUNet_raw, 'Dataset001_BloodVessel_2D/imagesTs'),
        "/home/user/Kaggle/Data/to_predict/kidney_5/images",
#         os.path.join(nnUNet_raw, 'Dataset001_BloodVessel_2D/imagesTs_pred2d'),
        None,
